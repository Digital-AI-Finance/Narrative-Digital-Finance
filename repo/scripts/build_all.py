...
from generate_dmp import load_dmp_content, generate_dmp_html, generate_comparison_html
...
def step_generate_dmp(
    logger: logging.Logger,
    dry_run: bool = False
) -> List[Path]:
    """Step 5: Generate DMP pages from single source JSON."""
    logger.info("=" * 60)
    logger.info("STEP 5: Generating DMP Pages")
    logger.info("=" * 60)

    dmp_json = DATA_DEST / "dmp-content.json"

    if not dmp_json.exists():
        logger.warning(f"DMP content file not found: {dmp_json}")
        return []

    if dry_run:
        logger.info("[DRY RUN] Would generate DMP pages in: %s", TARGET_REPO)
        return []

    try:
        content = load_dmp_content(dmp_json)

        # Generate main DMP page
        dmp_path = TARGET_REPO / "dmp.html"
        dmp_html = generate_dmp_html(content)
        dmp_path.write_text(dmp_html, encoding='utf-8')
        logger.info(f"  -> {dmp_path.name}")

        # Generate comparison page
        comparison_path = TARGET_REPO / "dmp-comparison.html"
        comparison_html = generate_comparison_html(content)
        comparison_path.write_text(comparison_html, encoding='utf-8')
        logger.info(f"  -> {comparison_path.name}")

        return [dmp_path, comparison_path]

    except Exception as e:
        logger.error(f"Failed to generate DMP pages: {e}")
        return []
...
def build_all(
    ...
    try:
        ...
        # Step 5: Generate DMP pages
        dmp_pages = step_generate_dmp(
            logger,
            dry_run=dry_run
        )
        generated_pages.extend(dmp_pages)
