...
The first phase consists of setting an initial research query based on specific terms present in title, abstract or keywords, using the advanced research function from Scopus. The query, given in appendix \ref{appendixC} - query \ref{lst:search_query_1}, aims to retrieve relevant publications related to (\textit{narrative} \textbf{AND} \textit{narrative}) \textbf{OR} (\textit{NLP} \textbf{AND} \textit{Financial Markets Dynamics}) \textbf{OR} (\textit{Narrative Economics} \textbf{OR} Financial Narrative Processing).
...
The second phase consists of applying some additional filters to get more homogeneous research. In our case, we complete the first query by some conditions on publication time (publication year should be greater or equal to 2010), document type (only articles) and language (only english written papers). The second full query is given in \ref{appendixC} - query \ref{lst:search_query_2}.

Using Python, this review sources the publications from the Scopus database, specifically retrieving the results basic information trough the API and retrieving the abstract, authors and keywords. We also use the same programming language to get the journal ranking from Scientific Journal Ranking website.
...
These statements define the scope and objectives of the systematic literature review and serve as filtering criteria in the selection process. 

To evaluate how well each paper aligns with these statements, a Transformer-based sentence embedding model is used, implemented via the Hugging Face `SentenceTransformer` library in Python. The statements are converted into embeddings, while the title, abstract, and keywords of each paper are retrieved from the Scopus API and concatenated into a single textual representation, which is also embedded. The cosine similarity between each paper's embedding and the five predefined statements is then computed, yielding five similarity scores per paper.
...
The process is conducted in two stages. First, an initial PCA is applied to determine the explained variance across components. The proportion of variance captured by each principal component is analyzed to assess how many dimensions are needed to retain the essential structure of the data. In this study, the results showed that the first three components account for 95\% of the total variance, indicating that a significant portion of the information contained in the original five similarity scores can be preserved in a three-dimensional space.

In the second stage, a final PCA transformation is applied, reducing the data to three components. This step ensures that redundant information is removed while maintaining the underlying structure that distinguishes relevant from non-relevant papers. By mapping the research papers onto this reduced space, the classification process becomes more robust and interpretable, as it prevents the inclusion of noise and redundant variance that could distort the clustering stage.

The reduced representation allows for a more efficient clustering process in the subsequent step. Papers that initially had similar similarity profiles across multiple research statements are now grouped based on their principal components, which capture the most distinguishing characteristics of their textual content. This transformation helps refine the classification process and ensures that only the most relevant dimensions of similarity contribute to the selection framework.


\subsubsection{Research clustering}

After reducing the dimensionality of the similarity scores, the next step in the selection process is clustering the papers into distinct relevance groups. This classification is performed using K-means clustering, a widely used unsupervised learning algorithm that partitions data into a predefined number of clusters based on similarity measures. The clustering is conducted in the three-dimensional space obtained from the PCA transformation, ensuring that only the most meaningful components contribute to the classification process.

To assign relevance levels, the clustering algorithm groups papers based on their average similarity score across the predefined research statements. The mean similarity score within each cluster is then computed to establish an order of relevance, where clusters with higher average similarity scores contain papers that align more closely with the systematic review criteria. The selection framework allows flexibility in defining the number of clusters, depending on the size and diversity of the retrieved papers.

Two main configurations can be used. When the dataset contains studies with varied methodologies, objectives, or data sources, a three-cluster approach is preferred. In this case, the papers are categorized into high, medium, and low relevance groups. The user can then decide whether to manually review the medium-relevance category to refine the selection or, if the initial query retrieved an excessive number of papers, to retain only those classified as high relevance. This configuration is particularly useful when there is a risk of including a large number of marginally relevant studies.

For more homogeneous datasets, where the retrieved studies share strong similarities in terms of methodology or scope, a two-cluster classification may be more appropriate. This approach separates the papers into high and low relevance categories, ensuring that only the most relevant studies are retained while reducing the risk of false positives. In cases where relevant literature is scarce or when a more exhaustive review is desired, the user can review the low-relevance cluster to verify whether any valuable studies were inadvertently excluded.

In our study, the initial search phase retrieved a large number of papers, which exhibited considerable methodological diversity. Given the size and heterogeneity of the dataset, we opted for a three-cluster configuration and retained only the high-relevance category. The final selection ensures that the included studies strongly align with the research objectives while minimizing the inclusion of marginally relevant literature.

\subsection{Data extraction}

Once the final set of papers was selected, the next step was to systematically extract relevant information. This phase was conducted manually to ensure accuracy and consistency in the reported findings. The initial dataset included studies retrieved from various academic sources; however, some papers were unavailable from standard repositories such as Elsevier, ArXiv, and SSRN. These studies were excluded from the analysis, leaving a final selection of 27 papers.

To structure the extraction process, a reporting framework was designed to capture key aspects of each study. This framework includes details on the research purpose, methodology, data sources, dataset characteristics, main findings, and practical implications. Each selected paper was analyzed using this standardized template to maintain consistency and facilitate comparisons across studies.

The purpose of this step is to document how each study contributes to the understanding of financial narratives, particularly in terms of narrative modeling techniques, data sources, and the integration of NLP methodologies. The structured extraction ensures that all studies are evaluated using the same criteria, minimizing potential biases in the review process.

While the current extraction process remains manual, future work aims to explore the possibility of automating parts of the extraction using NLP techniques. Automation could improve efficiency and scalability, allowing for the rapid processing of a larger number of studies. However, given the complexity of financial narrative research and the nuances in methodology descriptions, manual review remains essential to ensure accuracy and proper interpretation of findings.
...
- Get a clear definition of narratives in financial markets (financial narratives)
- Provide an overview on the financial narrative approaches in the literature, and how they are processed and modeled
- highlight any gap in that context

\subsection{Main findings}
- Algorithmic literature review: 
    - interesting base to automate the paper selection process (from 171 papers to 27), provides good results (only papers related to textual analysis in the context of economy/finance and/or economics/financial narratives)
- Financial Narrative:
    - lack unified definition
    - We got various research approaches: narrative theory, narratives processing or narrative modeling.
    - Narrative processing and modeling are done via different textual analysis approach: manual or NLP based (review of NLP techniques)
    - Various tasks: directly extract certain narratives (manual) or (NLP tasks) sentiment analysis, emotion analysis, topic modeling, summarization.
    - Most of the research do sentiment analysis (via bag-of-words/N-grams frequencies) and topic modeling (via LDA or its variants), only a few apply the most recent advancements using deep-learning transformers models
- Research gaps: no definition for financial narratives (a good proposition from \textcite{roos_narratives_2024} about collective economic narrative, too generic), too few research apply recent techniques that proved high efficiency for textual analysis, often reducing narratives to sentiment and/or topic/set of topics without studying for narrative strength or semantic coherence.

\subsection{Limitations and future recommendations}
- A few misclassifications compared to manual screening (acceptable)
- Still some papers that aren't relevant in the research context (theoretical papers and more generic  economics papers, like retail banking and business cycles)
-  To improve the algo selection process: improve initial query, improve statements (similar to prompt engineering), better Transformers model (advanced one), improving clustering method (HBDSCAN like \textcite{grootendorst_bertopic_2022} does)
...
This research was funded in whole or in part by the Swiss National Science Foundation (SNSF) IZCOZ0-213370. For the purpose of Open Access, a CC BY public copyright license is applied to any Author Accepted Manuscript (AAM) version arising from this submission.
